const data = [
  ...
  const total = Math.ceil(data.length / items);

  for (let i = 1; i <= total; i++) {
    const li = document.createElement("li");
    li.textContent = i;
    li.addEventListener("click", () => Display(i));
  ...
  `;
      itemsContainer.appendChild(personElement);
    }

    update(pageNo);
  }

  function update(activePage) {
    for (let i = 0; i < pagination.children.length; i++) {
      const li = pagination.children[i];
      if (parseInt(li.textContent) === activePage) {
